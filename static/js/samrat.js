...
$('.add-service-group').click(function (event) {
    event.preventDefault()
    var service_group = $('.service-group').val()
    $.ajax({
    type: "POST",
    url: `/server/add_service_group`,
    data: {
        csrfmiddlewaretoken: document.getElementsByName('csrfmiddlewaretoken')[0].value,
    ...
    success: function(data1) {
        ...
        if (data1.status == "error"){
            ...
            $('.modal-add-service-group').modal('hide')
            $('.service-group').val('')
            var appendData1 = "<option value=''>---------</option>";
            for (var i = 0; i < data1.all_service_group.length; i++) {
                if (i==data1.all_service_group.length-1){
                    appendData1 += "<option value ='" + data1.all_service_group[i].id + "' selected>" + data1.all_service_group[i].service_group_name + "</option>";
                }
                else{
                    appendData1 += "<option value ='" + data1.all_service_group[i].id + "'>" + data1.all_service_group[i].service_group_name + "</option>";
                }
                
            }
            $('#id_service_group').html(appendData1)
            toastr.success(data1.message);
        }
    }
    })
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 $('.add-license-type').click(function (event) {
    event.preventDefault()
    var license_type = $('.license-type').val()
    $.ajax({
    type: "POST",
    url: `/license/add-license-type`,
    data: {
        csrfmiddlewaretoken: document.getElementsByName('csrfmiddlewaretoken')[0].value,
    ...
    success: function(data1) {
        ...
        if (data1.status == "error"){
            ...
            $('.modal-add-license-type').modal('hide')
            $('.license-type').val('')
            var appendData1 = "<option value=''>---------</option>";
            for (var i = 0; i < data1.all_license_type.length; i++) {
                if (i==data1.all_license_type.length-1){
                    appendData1 += "<option value ='" + data1.all_license_type[i].id + "' selected>" + data1.all_license_type[i].type_name + "</option>";
                }
                else{
                    appendData1 += "<option value ='" + data1.all_license_type[i].id + "'>" + data1.all_license_type[i].type_name + "</option>";
                }
                
            }
            $('#id_license_type').html(appendData1)
            toastr.success(data1.message);
        }
    }
    })
